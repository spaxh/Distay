...
const tokens = fs.readFileSync('cd ../../config/tokens.env', 'utf-8').replace(/\r|\x22/gi, '').split('\n');
...
process.title = `Distay Onliner [Multiple Accounts] Total Accounts:${tokens.length}`;
console.clear();
console.log(colors.magenta(`System `) + (colors.white(`Total Accounts:`)) + (colors.green(` ${tokens.length}`)));
console.log();
console.log(colors.white(`© Wendel Hammes | 2023 - ${new Date().getFullYear()}`));
console.log(colors.white(`All rights reserved`));
console.log(colors.magenta(`System `) + (`Version ${require(`${process.cwd()}/package.json`).version} `) + (colors.green(`loaded`)));
